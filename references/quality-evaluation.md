...
## 评测服务于当前 benchmark 的满分目标

先用用户实际评分规则定义满分及剩余差距；没有另定目标时无需再询问目标值。缺少当前成绩、详细结果或规则，应读取已有材料或直接向用户索取。没有测过就通过原生入口建立基线，不用合成示例替用户生成成绩。

有方向的参数和组合试验也使用这条质量路线：少量有区别的候选先筛选，有效区域再细化，成本低且影响广的改动可以直接运行完整原生套件。不要为了形式上的分层，把本来很便宜的一次真实评测拆成多轮。

验证范围围绕用户指定的 benchmark。若它是固定公开套件，可冻结候选后完整复跑并报告该套件结果；这些案例参与过选择就如实标记，不能宣称未见场景泛化。没有额外保留集不禁止继续优化或取得官方成绩。若目标包含隐藏场景或随机故障，则保留相关独立实例/运行条件来检查选择偏差；代理验证再好也不等于隐藏集已满分。确认目标达成需要什么证据，取决于实际规则，不给用户额外加上跨 benchmark 通用最优的任务。
