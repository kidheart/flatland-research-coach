...
**审阅既有证据 → 提出假设 → 审查实验 → 做小改动 → 实际运行与评测 → 继续、修正或回滚**

先核实目标、硬约束、评测规则和可用资源，复用已有有效结果，再选择能回答当前问题的有界本地实验。每轮将结果绑定到实际版本，记录完成情况、失败、超时与资源消耗；退步或违反约束时回滚本轮改动，保留最好已验证版本。达到已验证的目标、用尽预算，或已没有值得在剩余预算内检验的可行假设时，汇总结果并停止。

在已授权的本地范围和预算内，agent 可以连续完成这些步骤，无需每轮等待使用者回答或重复批准。只追问无法从现有材料合理推断、且会阻止有效推进的关键缺失事项。

宿主支持真实多 agent 工具时，可以委派研究者提出方案、审查者质疑假设与比较设计、执行者实施和测量，让 agent 之间根据实际产物与结果协作。没有委派工具时，由单个 agent 按这些职责自审，并如实说明；不得把单个 agent 写出的角色对话当作已经发生的独立审查。
...
先审阅现有代码、规则、日志和可核实的历史结果，确认当前最好已验证版本。
在上述范围和预算内，自行提出假设、审查实验设计、做有界的小改动，
运行必要的检查与真实评测，再按证据继续、修改或回滚本轮改动。
